...
rem SysWOW64 exists only on 64-bit Windows. Uninstall the native provider.
if exist "%SystemRoot%\SysWOW64\" (
    set "DLL=%CD%\x64\DefThumbnailProvider.dll"
    set "REGSVR=%SystemRoot%\System32\regsvr32.exe"
    echo Detected 64-bit Windows. Uninstalling the x64 thumbnail provider...
) else (
    set "DLL=%CD%\x86\DefThumbnailProvider.dll"
    set "REGSVR=%SystemRoot%\System32\regsvr32.exe"
    echo Detected 32-bit Windows. Uninstalling the x86 thumbnail provider...
)
...
if "%RESULT%"=="0" (
    ...
    echo ERROR: Uninstallation failed with code %RESULT%.
)
