...
begin
  ...
  with SaveDialog2 do
    case i of
      ...
      2:
      begin
        FileName:= ExtractPath + Node.Text + '.bmp'; // <-- FIXED
      ...
      0:
        ...
        begin
          ...
          begin
            ...
            if Bmp<>nil then
              Bmp.SaveToFile(s + it[j].Text + '.bmp'); // <-- FIXED
            FreeAndNil(Bmp);
          end;
        end;
      1:
        ...
        begin
          ...
          if (Bmp<>nil) and Extractin24bits1.Checked then
            Bmp.PixelFormat:= pf24bit;
          if Bmp<>nil then
            Bmp.SaveToFile(s + Node[j].Text + '.bmp'); // <-- FIXED
          FreeAndNil(Bmp);
        end;
      2:
      begin
...
begin
  if fileName='' then exit;
  SaveDialog3.FileName:= ExtractPath + ExtractFileName(fileName) + '.h3l'; // <-- FIXED
  ...
  with Def do
    for i:=0 to length(Groups)-1 do
    begin
      f2:=f2+Dir+ItemNames[i][0]+'.bmp'; // <-- FIXED
      for j:=1 to Groups[i].ItemsCount-1 do
        f2:=f2+#13#10+Dir+ItemNames[i][j]+'.bmp'; // <-- FIXED
      if New then
